...
cmd:option('-batchSizeHessian', 1, 'mini-batch size (1 = pure stochastic)')
cmd:option('-weightDecay', 0e-2, 'weight decay (SGD only)')
...
cmd:option('-maxEpochHessian', 5, 'maximum number of epochs to run')
...
cmd:option('-iterMethodDelta', 10e-10, 'threshold to stop powermethod; will keep running until difference between norm_Hd and norm_Hd_old < powermethodDelta')
...
cmd:option('-lineSearch',false, 'turn on newton-like stepsize')
cmd:option('-iterationMethod','Power', 'Power iteration / Lanczos iteration')
cmd:text()
opt = cmd:parse(arg)
...
local iterationMethods_filepath = opt.currentDir .. '/iterationMethodsCifar.lua'
...
function trainSGD(dataset)
   ...
   local filename = paths.concat(opt.save, 'cifar_G.net')
...
---------------------------------------------------------------------------------------------------------------------------------------
      -- create mini batch
      local inputs = torch.Tensor(opt.batchSizeHessian,3,geometry[1],geometry[2])
      local targets = torch.Tensor(opt.batchSizeHessian)
      local k = 1
      for i = t,math.min(t+opt.batchSizeHessian-1,dataset:size()) do
         -- load new sample
         local input_ = dataset.data[i]
         local target_ = dataset.labels[i]
      ...
      local feval = function(x)
         ...
         --]]
         -- update confusion
         for i = 1,opt.batchSizeHessian do
...
---------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 local doGradStep = 1
	 	...
	 	if torch.norm(gradParameters) < opt.gradNormThresh then
		     ...
		     if opt.iterationMethod =="power" then  
			 minEigValH, v, converged2 = negativePowermethodClassifier(inputs,targets,parameters:clone(),opt.iterMethodDelta,opt.currentDir,opt.modelpath,maxEigValH)
		     end
		     if opt.iterationMethod =="lanczos"  then 
   ...
   -- train error
   trainError = trainError / math.floor(dataset:size()/opt.batchSizeHessian)
   --trainError = trainError /dataset:size()
   ...
   -- save/log current net
   local filename = paths.concat(opt.save, 'cifar_H.net')
   os.execute('mkdir -p ' .. paths.dirname(filename))
   if paths.filep(filename) then
      os.execute('mv ' .. filename .. ' ' .. filename .. '.old')
   end
...
while flag do
   -- train/test
   trainAcc, trainErr = trainSGD(trainData)
...
while flag do
   -- train/test
   trainAcc, trainErr = trainHes(trainData)
   testAcc,  testErr  = test (testData)
